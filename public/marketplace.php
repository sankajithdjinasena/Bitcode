...
// ── Track which specific ITEM IDs this user has in cart or already ordered ──
// Per-item check (NOT per-event): user can still buy other items in the same event.
// Only the exact item they already picked is locked.
$takenStmt = $conn->prepare("
    SELECT item_id FROM cart   WHERE user_id = ?
    UNION
    SELECT item_id FROM orders WHERE user_id = ? AND status = 'confirmed'
");
$takenStmt->execute([$userId, $userId]);
$takenItems = array_flip($takenStmt->fetchAll(PDO::FETCH_COLUMN)); // hash-set O(1) lookup
// ─────────────────────────────────────────────────────────────────────────────
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .event-hero-overlay {
            position: absolute;
            inset: 0;
            background: linear-gradient(to right, rgba(7,7,13,0.85) 0%, rgba(7,7,13,0.45) 55%, rgba(7,7,13,0.05) 100%);
            display: flex;
        ...
        .event-hero-time { font-size: 12px; color: var(--muted2); font-weight: 300; }

        .status-tag {
            display: inline-flex; align-items: center; gap: 5px;
            padding: 3px 10px; border-radius: 100px; font-size: 11px; font-weight: 500; flex-shrink: 0;
        }
        ...
        .price { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; }

        .stock-wrap { text-align: right; }

        .stock-num { font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 800; }
        ...
        .stock-bar-track {
            ...
            border-radius: 100px; margin-top: 6px; width: 64px; margin-left: auto;
        }

        .stock-bar-fill { height: 100%; border-radius: 100px; transition: width 0.5s; }

        .countdown-box {
            display: flex; align-items: center; gap: 9px; padding: 9px 12px;
            background: rgba(255,140,66,0.05); border: 1px solid rgba(255,140,66,0.15);
            border-radius: 8px; color: var(--accent2);
        }

        .countdown-sub {
            font-size: 10px; color: var(--muted2); margin-bottom: 2px;
            text-transform: uppercase; letter-spacing: 0.5px;
        }

        .countdown-val { font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 800; }

        .btn-buy {
            width: 100%; padding: 0.7rem 1rem; background: var(--accent); border: none;
            border-radius: 9px; color: #fff; font-family: 'Syne', sans-serif;
            font-size: 13px; font-weight: 700; letter-spacing: 0.2px;
            cursor: pointer; transition: all 0.2s; margin-top: auto;
        }

        .btn-buy:hover:not(:disabled) {
            background: #ff6635; box-shadow: 0 4px 18px rgba(255,77,28,0.35); transform: translateY(-1px);
        }

        .btn-buy:disabled { background: rgba(255,255,255,0.04); color: var(--muted); cursor: not-allowed; }

        .btn-soldout {
            width: 100%; padding: 0.7rem 1rem; background: rgba(239,68,68,0.05);
            border: 1px solid rgba(239,68,68,0.15); border-radius: 9px; color: #fca5a5;
            font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700;
            cursor: not-allowed; margin-top: auto;
        }

        /* Only the specific purchased item shows this — not all items in the event */
        .btn-incart {
            width: 100%; padding: 0.7rem 1rem; background: rgba(34,197,94,0.06);
            border: 1px solid rgba(34,197,94,0.2); border-radius: 9px; color: #86efac;
            font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700;
            cursor: pointer; margin-top: auto; transition: all 0.2s;
            text-decoration: none; display: block; text-align: center;
        }

        .btn-incart:hover { background: rgba(34,197,94,0.12); border-color: rgba(34,197,94,0.35); }
        ...
        #toast {
            ...
            padding: 1rem 1.5rem; border-radius: 12px; font-size: 14px; font-weight: 500;
            max-width: 320px; opacity: 0; transform: translateY(8px); transition: all 0.3s; pointer-events: none;
        }
    ...
    <div class="event-block">
        ...
        <div class="items-row">
            <?php foreach ($event['items'] as $item):
                ...
                // Check per ITEM — not per event
                $thisItemTaken = isset($takenItems[$item['item_id']]);
            ?>
            <div class="item-card">
                ...
                <?php elseif ($thisItemTaken): ?>
                    <!-- Only THIS exact item is locked; other items in the event are unaffected -->
